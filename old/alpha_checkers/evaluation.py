...
def simulate_game(filepath: str, mcts_iterations: int = 1000) -> int:
    ...
    while not game.is_game_over():
        if agent == 1:
            start_pos, moves = mcts_agent.get_best_move(
                iterations=mcts_iterations)
            game.make_move(start_pos, moves)
            print(start_pos, moves)
            agent *= -1
...
def evaluate_mcts_vs_random(filepath: str, num_simulations: int, mcts_iterations: int = 1000) -> None:
    ...
    for num_games_so_far in tqdm(range(num_simulations), desc="Simulating games"):
        ...
        with open(f"{filepath}.txt", 'a') as file:
            file.write(
                f"MCTS wins: {mcts_wins} ({(mcts_wins / (num_games_so_far + 1)) * 100:.2f}%) \n")

    print(f"Results after {num_simulations} simulations:")
    print(
        f"MCTS Wins: {mcts_wins} ({(mcts_wins / num_simulations) * 100:.2f}%)")
    print(
        f"Random Wins: {random_wins} ({(random_wins / num_simulations) * 100:.2f}%)")
    ...
    with open(f"{filepath}.txt", "a") as file:
        ...
        file.write(
            f"Random Wins: {random_wins} ({(random_wins / num_simulations) * 100:.2f}%)\n")
        file.write(
            f"Draws: {draws} ({(draws / num_simulations) * 100:.2f}%)\n")


if __name__ == "__main__":
    num_simulations = 50  # Set the number of simulations
    evaluate_mcts_vs_random('exp1', num_simulations, mcts_iterations=1000)
